...
cnx = mysql.connector.connect(user=user, password=pwd,
                              ...
                              database=stage_db)    
# CSV file path
customer_file_path = '/opt/airflow/directory/sideprojects/carly/input/customers.csv'
df = pd.read_csv(customer_file_path)

table_name='customer'
cursor = cnx.cursor()
for _, row in df.iterrows():
    insert_query = "INSERT INTO {} (email, customer_id, country) VALUES ('{}', '{}', '{}')".format(
        table_name, row['email'], row['customer_id'], row['country'])
